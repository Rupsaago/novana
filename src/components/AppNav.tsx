...
// ═══════════════════════════════════════════════════════════════════════════
// AppNav — handles navigation for all dashboard pages.
//
// MOBILE:  bottom tab bar (like Instagram/TikTok)
// DESKTOP: left sidebar (like before)
// ═══════════════════════════════════════════════════════════════════════════
...
// ── Tab config — order matters, this is left→right on mobile ─────────────────
const TABS = [
        ...
        <div className="flex items-center gap-3 px-4 py-3 rounded-2xl">
          ...
          <button
            ...
            className="text-nova-muted/50 hover:text-nova-rose transition-colors
                       p-1 rounded-lg hover:bg-nova-card text-sm"
          >
            ⎋
          </button>
        </div>
...
// ── Mobile top bar (just logo + user avatar, no hamburger) ───────────────────
function MobileTopBar({ user }: { user: User | null }) {
  ...
  return (
    <header className="lg:hidden flex items-center justify-between
      ...
      {/* Page title — centered */}
      <span className="font-display text-nova-text text-sm absolute left-1/2
                       -translate-x-1/2">
        {pageTitle}
      </span>
        ...
        return (
          <Link
            ...
            className="flex-1 flex flex-col items-center justify-center
                       gap-1 py-3 px-2 transition-all duration-200 relative"
          >
            ...
            <span className={`text-[10px] font-medium leading-none transition-colors
                             ${isActive ? 'text-nova-purple' : 'text-nova-muted/60'}`}>
              {tab.label}
            </span>
          </Link>
        )
